...
const THEME = {
    ...
    gradientSimpson: ['#ff6b35', '#ff8c42', '#ffa600'], // Bright orange
...
export default function DevotionalCarousel() {
    ...
    const loadPreferenceAndBuildCards = useCallback(async () => {
        try {
            ...
            // Load christian detail config (defaults = all true)
            const christianDetailJson = await AsyncStorage.getItem('christianDetailConfig');
            const christianDetail = christianDetailJson ? JSON.parse(christianDetailJson) : {};
            const spurgeonOn = christianDetail.spurgeon !== false;
            const simpsonOn = christianDetail.simpson !== false;
            ...
            // SIMPSON (Christian or Both) - FIRST CARD
            if ((pref === 'christian' || pref === 'both') && simpsonOn) {
                const dayOfYear = Math.floor((today - new Date(today.getFullYear(), 0, 0)) / (1000 * 60 * 60 * 24));
                const simpsonReading = simpsonData.find((item) => item.day === dayOfYear);

                if (simpsonReading) {
                    builtCards.push({
                        id: 'simpson',
                        type: 'christian',
                        title: 'Simpson Devotional',
                        subtitle: simpsonReading.verse.slice(0, 60) + '...',
                        preview: simpsonReading.body.slice(0, 120) + '...',
                        gradient: THEME.gradientSimpson,
                        icon: 'book',
                        onPress: () => navigation.navigate('SimpsonReading'),
                    });
                }
            }

            // SPURGEON (Christian or Both) - SECOND CARD
            if ((pref === 'christian' || pref === 'both') && spurgeonOn) {
                    ...
                    >
                        ...
                        >
                            <View style={styles.cardHeader}>
                                ...
                                <View style={styles.iconBadge}>{renderIcon(card.icon)}</View>
                            </View>
