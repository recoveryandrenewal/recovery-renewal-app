// Screens/StoicReadingScreen.jsx
...
const THEME = {
  ...
  accentCyan: '#00E5FF',
  accentMarcus: '#8B5CF6',     // Royal purple - Marcus Aurelius
  accentEpictetus: '#A78BFA',  // Lighter purple - Epictetus
  textWhite: '#FFFFFF',
  textGray: '#A0A0B0',
  textSoft: '#C0C0D0',
};
...
export default function StoicReadingScreen({ route, navigation }) {
  ...
  const [currentPhilosopher, setCurrentPhilosopher] = useState('marcus'); // 'marcus' or 'epictetus'
  ...
  useEffect(() => {
    loadReading(currentMonth, currentDay, currentPhilosopher);
  }, [currentMonth, currentDay, currentPhilosopher]);

  const loadReading = (month, day, philosopher) => {
    const dataSource = philosopher === 'marcus' ? marcusData : epictetusData;
    const entry = dataSource.find(
  ...
  const togglePhilosopher = () => {
    const newPhilosopher = currentPhilosopher === 'marcus' ? 'epictetus' : 'marcus';
  ...
  const getAccentColor = () => {
    return currentPhilosopher === 'marcus' ? THEME.accentMarcus : THEME.accentEpictetus;
  };

  const getPhilosopherName = () => {
    return currentPhilosopher === 'marcus' ? 'Marcus Aurelius' : 'Epictetus';
  };

  const getToggleButtonText = () => {
    return currentPhilosopher === 'marcus' ? 'Marcus' : 'Epictetus';
  };
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={[styles.headerSubtitle, { color: getAccentColor() }]}>
              Stoic Wisdom
            </Text>
          </View>
        ...
        >
          ...
          >
            ...
            <View style={styles.referenceContainer}>
              <Text style={[styles.philosopherName, { color: getAccentColor() }]}>
                {getPhilosopherName()}
              </Text>
              <Text style={styles.referenceText}>
                {currentPhilosopher === 'marcus'
                  ? `Meditations ${reading.book}:${reading.section}`
                  : `Enchiridion ${reading.chapter}`
                }
              </Text>
            </View>

            <View style={[styles.divider, { backgroundColor: getAccentColor() }]} />
        ...
        <View style={styles.navFooter}>
          ...
          >
            ...
            <TouchableOpacity onPress={togglePhilosopher} style={styles.toggleButton}>
              <Text style={[styles.toggleButtonText, { color: getAccentColor() }]}>
                {getToggleButtonText()}
              </Text>
            </TouchableOpacity>
