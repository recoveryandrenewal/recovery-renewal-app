...
const BibleScreen = ({ navigation }) => {
  ...
  // Combined Results for Search with Smart Sort
  const combinedResults = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return [];

    // 1. Get all matches and track original biblical index
    const matches = kjvData.books
      .map((book, index) => ({ ...book, originalIndex: index }))
      .filter(book => book.name.toLowerCase().includes(q));

    // 2. Sort by "Starts With" priority, then by original index
    return matches.sort((a, b) => {
      ...
      // Priority 2: Original Biblical Order
      return a.originalIndex - b.originalIndex;
    });
  }, [searchQuery]);

  // Quick Navigate Detection
  const quickNavigate = useMemo(() => {
    ...
    if (match) {
      ...
      const foundBook = kjvData.books.find(b => b.name.toLowerCase() === bookName || b.name.toLowerCase().startsWith(bookName));
      if (foundBook) {
        const hasChapter = foundBook.chapters.some(ch => ch.chapter === chapterNum);
        if (hasChapter) {
          return { book: foundBook, chapter: chapterNum };
        }
      }
    }
    return null;
  ...
  const handleQuickNavigate = (nav) => {
    setSelectedBook(nav.book);
    setSelectedChapter(nav.chapter);
    setViewMode('verses');
    setSearchQuery('');
  };
  ...
  const renderBookSelector = () => (
    ...
    >
      ...
      {/* Logic for Unified Search Results vs Categorized Sections */}
      {searchQuery.length > 0 ? (
        <View style={styles.section}>
          {/* Quick Navigate Action */}
          {quickNavigate && (
            <TouchableOpacity 
              style={[styles.bookCard, styles.quickNavCard]} 
              onPress={() => handleQuickNavigate(quickNavigate)}
            >
              <View style={styles.quickNavIcon}>
                <Zap size={20} color={THEME.textWhite} />
              </View>
              <View style={styles.bookCardContent}>
                <Text style={[styles.bookTitle, { color: THEME.accentCyan }]}>
                  Go to {quickNavigate.book.name} {quickNavigate.chapter}
                </Text>
                <Text style={styles.bookSubtitle}>Quick Navigate</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  quickNavCard: { borderColor: THEME.accentCyan, borderStyle: 'dashed', backgroundColor: 'rgba(0, 229, 255, 0.08)' },
  quickNavIcon: { width: 36, height: 36, borderRadius: 18, backgroundColor: THEME.accentCyan, alignItems: 'center', justifyContent: 'center', marginRight: 16 },
